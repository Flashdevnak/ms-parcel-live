...
function scheduleFromExpiry(expiresAt) {
  ...
  clearTimeout(retryTimer);
  const wait = Math.max(15_000, nextCheckAt - Date.now());
  retryTimer = setTimeout(() => {
    if (!document.hidden) void loadFullAnalytics(false);
  }, wait);
}
...
async function loadFullAnalytics(forceRead = false) {
  ...
  try {
    ...
    if (fresh) {
      lastBranch = id;
      scheduleFromExpiry(cached.expires_at);
      return;
    }

    const leader = await claim(id, token);
    if (!leader) {
      nextCheckAt = Date.now() + 1800;
      retrySoon(1800);
      return;
    }
...
function init() {
  ...
  // Manual refresh re-reads the shared cache; it still respects a fresh full snapshot.
  ...
  document.addEventListener('visibilitychange', () => {
    if (!document.hidden && Date.now() >= nextCheckAt) retrySoon(300);
  });

  setInterval(() => {
    if (!document.hidden && Date.now() >= nextCheckAt) void loadFullAnalytics(false);
  }, 60_000);
}
