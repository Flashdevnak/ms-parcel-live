...
assert.equal(await claimAnalyticsLease(db,1,'late-user',at+179999),null);
const successor = await claimAnalyticsLease(db,1,'successor',at+180001);
...
assert.ok(Date.parse(records.get('b:1:engine:full-analytics').lease_until)>at+180001);
await successor();
assert.equal(typeof await claimAnalyticsLease(db,1,'next',at+180002),'function');
...
console.log('PASS simulated atomic lease: 40 clients / 2 branches = 2 leaders, 180s crash expiry, late release fenced, branch keys isolated, DB failure stops scan');
