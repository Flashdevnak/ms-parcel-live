...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(req.method==='GET'&&route==='analytics'){
      ...
      if(c?.payload?.schemaVersion===6&&new Date(c.expires_at).getTime()>Date.now())return json({ok:true,data:c.payload,meta:{cache:'hit',expiresAt:c.expires_at,ttlMs:Math.max(0,new Date(c.expires_at).getTime()-Date.now()),branchId}});
      const conn=await getConnection(branchId);
      try{
