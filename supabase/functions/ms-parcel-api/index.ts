...
const ANALYTICS_SCHEMA_VERSION = 10;
const analyticsCompatible = (payload: any) => Number(payload?.schemaVersion || 0) === ANALYTICS_SCHEMA_VERSION;
...
Deno.serve(async (req) => {
  ...
  try {
    if (req.method === 'POST' && route === 'login') {
      ...
      const rows = await db(`app_profiles?username=ilike.${encodeURIComponent(username)}&select=email,access_status&limit=1`);
      const p = rows?.[0];
      if (!p?.email) return json({ ok: false, message: 'ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง' }, 401);
      let session;
      try { session = await authPasswordLogin(String(p.email), password); }
      catch { return json({ ok: false, message: 'ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง' }, 401); }
    ...
    if (req.method === 'GET' && route === 'health') {
      if (profile.role !== 'admin' || profile.access_status !== 'active') return json({ ok: false, message: 'Admin only' }, 403);
      return json({ ok: true, data: await systemHealth() });
    }
    ...
    if (req.method === 'GET' && route === 'status') {
      ...
      let branch: any = null;
      try { branch = await resolveBranch(profile, requested); } catch {}
      const conn = branch
        ? (await db(`ms_connection?branch_id=eq.${branch.id}&select=branch_id,label,store_id,store_name,credential_updated_at,last_ok_at,last_error&limit=1`))?.[0] || null
        : null;
      return json({ ok: true, data: { profile: publicProfile(profile), branch, branches, connection: conn } });
    }

    if (profile.access_status !== 'active') {
      return json({ ok: false, message: profile.access_status === 'disabled' ? 'บัญชีนี้ถูกระงับการใช้งาน' : 'บัญชีนี้ยังไม่เปิดใช้งาน' }, 403);
    }

    const requestedBranch = asBranchId(u.searchParams.get('branch_id'));
    const branch = await resolveBranch(profile, requestedBranch);
    ...
    if (req.method === 'POST' && route === 'har') {
      ...
      catch { return json({ ok: false, message: 'ไฟล์ HAR ไม่ใช่ JSON ที่ถูกต้อง' }, 400); }
      const found = pickHarRequest(har);
      const encrypted = await encryptCredential(found.credential);
      const existing = await db(`ms_connection?branch_id=eq.${branchId}&select=id&limit=1`);
      const patch = {
        branch_id: branchId,
        label: branch.code,
    ...
    if (req.method === 'GET' && route === 'summary') {
      const cacheKey = `b:${branchId}:summary:transfer-summary`;
      const cached = await db(`summary_cache?cache_key=eq.${encodeURIComponent(cacheKey)}&select=*&limit=1`);
      const c = cached?.[0];
      if (c && new Date(c.expires_at).getTime() > Date.now()) {
        return json({ ok: true, data: c.payload, meta: { cache: 'hit', expiresAt: c.expires_at, ttlMs: Math.max(0, new Date(c.expires_at).getTime() - Date.now()), branchId } });
      }
      ...
      try {
        ...
        const payload = { ...summary, sourceAt, branchId };
        if (c?.content_hash === hash) {
          await db(`summary_cache?cache_key=eq.${encodeURIComponent(cacheKey)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ source_updated_at: sourceAt, expires_at: expiresAt }) });
        } else {
        ...
        return json({ ok: true, data: payload, meta: { cache: 'miss', ttlMs: 60_000, branchId } });
      } catch (e) {
        const m = errMessage(e);
        await updateConnectionHealth(conn, false, m);
        if (c?.payload) return json({ ok: true, data: c.payload, meta: { cache: 'stale', stale: true, error: m, ttlMs: 15_000, branchId } });
        return json({ ok: false, message: m }, 502);
      }
    }

    if (req.method === 'GET' && route === 'analytics') {
      const cacheKey = `b:${branchId}:summary:full-analytics-v1`;
      const cached = await db(`summary_cache?cache_key=eq.${encodeURIComponent(cacheKey)}&select=*&limit=1`);
      const c = cached?.[0];
      if (analyticsCompatible(c?.payload) && new Date(c.expires_at).getTime() > Date.now()) {
        return json({ ok: true, data: c.payload, meta: { cache: 'hit', expiresAt: c.expires_at, ttlMs: Math.max(0, new Date(c.expires_at).getTime() - Date.now()), branchId } });
      }

      const release = await claimAnalyticsLease(db, branchId, authUser.id);
      if (!release) {
        if (c?.payload) return json({ ok: true, data: c.payload, meta: { cache: 'coalesced', stale: true, refreshing: true, ttlMs: 15_000, branchId } });
        return json({ ok: false, message: 'กำลังรวบรวมข้อมูลของ HUB นี้ กรุณารอสักครู่', meta: { cache: 'coalesced', refreshing: true, ttlMs: 15_000, branchId } }, 202);
      }

      try {
        const latest = (await db(`summary_cache?cache_key=eq.${encodeURIComponent(cacheKey)}&select=*&limit=1`))?.[0];
        if (analyticsCompatible(latest?.payload) && new Date(latest.expires_at).getTime() > Date.now()) {
          return json({ ok: true, data: latest.payload, meta: { cache: 'hit-after-lease', ttlMs: Math.max(0, new Date(latest.expires_at).getTime() - Date.now()), branchId } });
        }

        const conn = await getConnection(branchId);
        try {
          const analytics = await buildFullAnalytics(conn, branch, 10_000);
          const sourceAt = String(analytics?.updatedAt || new Date().toISOString());
          ...
          await db('summary_cache?on_conflict=cache_key', {
            ...
            body: JSON.stringify({ cache_key: cacheKey, branch_id: branchId, payload, content_hash: hash, source_updated_at: sourceAt, expires_at: expiresAt }),
          });
          await updateConnectionHealth(conn, true);
          return json({ ok: true, data: payload, meta: { cache: 'miss', expiresAt, ttlMs, branchId } });
        } catch (e) {
          const m = errMessage(e);
          const backoffMs = 5 * 60_000;
          const expiresAt = new Date(Date.now() + backoffMs).toISOString();
          await updateConnectionHealth(conn, false, m);
          if (c?.payload) {
    ...
    if (req.method === 'GET' && route === 'live') {
      ...
      const requestedSize = Number(u.searchParams.get('page_size') || 100);
      const pageSize = [20, 50, 100].includes(requestedSize) ? requestedSize : 100;
      const knownHash = String(u.searchParams.get('known_hash') || '').slice(0, 128);
      const cacheKey = `b:${branchId}:p:${page}:s:${pageSize}`;
      ...
      try {
        ...
        const oldHash = await oldHashForCache(c);
        const newHash = await hashPage(fresh.rows, fresh.total);
        const changed = !c?.content_hash || oldHash !== newHash;
        const streak = changed ? 0 : Number(c?.unchanged_streak || 0) + 1;
        const ttlMs = changed ? 8000 : (streak >= 5 ? 30000 : 15000);
        const expiresAt = new Date(Date.now() + ttlMs).toISOString();
        let delta: any = null;
        if (changed) {
          delta = oldHash ? diffRows(oldRows, fresh.rows) : null;
          const payload = { rows: fresh.rows, total: fresh.total, page, pageSize, changed: true, branchId };
          ...
          await db(`live_cache_pages?cache_key=eq.${encodeURIComponent(cacheKey)}`, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ source_updated_at: now, expires_at: expiresAt, item_count: fresh.rows.length, source_total: fresh.total, unchanged_streak: streak }) });
        }
        ...
        const m = errMessage(e);
        await updateConnectionHealth(conn, false, m);
        if (c?.payload) return liveResponseFromCache(c, knownHash, profile.role, 'stale', true, m);
        return json({ ok: false, message: m }, 502);
      }
    }
    ...
    return json({ ok: false, message: errMessage(e) }, 500);
  }
});
