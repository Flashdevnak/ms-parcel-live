...
export async function collectSnapshot(fetchPage, pageSize = 5000) {
  let requests = 0;
  let result;
  for (let attempt = 1; attempt <= 2 && requests < MAX_PAGES; attempt++) {
    ...
    const base = { total, requestedPageSize: pageSize, sourcePageSize: observed, pages, maxPages: MAX_PAGES, attempt };
