...
export async function buildFullAnalytics(conn:any,branch:any,requestedPageSize=DEFAULT_PAGE_SIZE){
  ...
  const consume=(rows:any[])=>{for(const row of rows){
    scanned+=1;const kg=weightKg(row?.store_weight),band=bandOf(row?.real_arrive_time,now),action=String(row?.LastAction_name||'-').trim()||'-',plan=parseTime(row?.plan_leave_time),overdue=Number.isFinite(plan)&&now>plan,critical=overdue&&band==='over48';
    totalWeightKg+=kg;bandTotals[band]=(bandTotals[band]||0)+1;addCount(actions,action);addCount(parcelStates,String(row?.state_name||'-').trim()||'-');if(overdue)overdueTotal+=1;if(critical)criticalTotal+=1;
  ...
  return{complete:scanned>=total,total,scanned,sourcePageSize:pageSize,pages,totalWeightKg:Math.round(totalWeightKg*1000)/1000,avgWeightKg:scanned?Math.round((totalWeightKg/scanned)*1000)/1000:0,baggedParcels,uniqueBags:bags.size,overdueTotal,criticalTotal,bandTotals,fd:sortedGroups(fd),lh:sortedGroups(lh),actions:sortedCounts(actions),parcelStates:sortedCounts(parcelStates),cells:[...cells.values()].map((x)=>({...x,w:Math.round(x.w*1000)/1000})),updatedAt:new Date().toISOString()};
}
